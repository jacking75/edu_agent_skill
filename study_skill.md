...
---
# 사례: 스킬 크리에이터 (Skill Creator)

새로운 스킬을 만들고 반복적으로 개선하기 위한 스킬입니다.

스킬 생성 과정을 대략적으로 설명하면 다음과 같습니다:

- 스킬이 무엇을 해야 하는지, 어떻게 동작해야 하는지 결정하기
- 스킬 초안 작성하기
- 몇 가지 테스트 프롬프트를 만들고, '스킬이 탑재된 Claude'로 실행해보기
- 사용자가 결과를 정성적·정량적으로 평가할 수 있도록 돕기
  - 실행이 백그라운드에서 진행되는 동안, 정량적 평가(evals)가 없다면 초안을 작성합니다(이미 있다면 그대로 사용하거나, 변경이 필요하다면 수정). 그런 다음 사용자에게 설명합니다(이미 존재하는 경우 기존 항목들을 설명).
  - `eval-viewer/generate_review.py` 스크립트를 사용하여 사용자가 결과를 확인하고 정량적 지표도 살펴볼 수 있도록 합니다.
- 사용자의 평가 결과 피드백을 바탕으로 스킬을 재작성합니다(정량적 벤치마크에서 명백한 결함이 발견된 경우도 포함).
- 만족할 때까지 반복합니다.
- 테스트 세트를 확장하고 더 큰 규모로 다시 시도합니다.

이 스킬을 사용할 때 여러분의 역할은 사용자가 이 과정의 어느 단계에 있는지 파악하고, 단계를 진행할 수 있도록 도와주는 것입니다. 예를 들어 사용자가 "X를 위한 스킬을 만들고 싶어요"라고 하면, 무슨 의미인지 좁혀나가고, 초안을 작성하고, 테스트 케이스를 작성하고, 어떻게 평가할지 결정하고, 모든 프롬프트를 실행하고, 반복하는 것을 도울 수 있습니다.

반대로 사용자가 이미 스킬 초안을 가지고 있다면, 바로 평가/반복 루프 단계로 넘어갈 수 있습니다.

물론 항상 유연하게 대응해야 하며, 사용자가 "많은 평가는 필요 없어요, 그냥 같이 느낌으로 해봐요"라고 하면 그렇게 해도 됩니다.

스킬이 완성된 이후(순서는 유연하게 조정 가능), 스킬 설명 개선기(skill description improver)를 실행하여 스킬 트리거링을 최적화할 수도 있습니다. 이를 위한 별도 스크립트가 있습니다.
...
스킬 크리에이터는 코딩 용어에 대한 친숙도가 매우 다양한 사람들이 사용할 수 있습니다. 최근 Claude의 강력함에 영감을 받아 배관공이 터미널을 열고, 부모님과 조부모님들이 "npm 설치 방법"을 검색하는 추세가 생겼습니다. 반면 대부분의 사용자는 꽤 컴퓨터에 익숙한 편입니다.

따라서 맥락 단서에 주의를 기울여 소통 방식을 조절해 주세요! 기본적인 기준을 예시로 들면:

- "평가(evaluation)"와 "벤치마크(benchmark)"는 경계선에 있지만 사용 가능합니다.
- "JSON"과 "assertion"은 사용자가 이를 알고 있다는 명확한 단서가 보일 때만 설명 없이 사용하세요.

확신이 없을 때는 용어를 간략히 설명해도 됩니다. 사용자가 이해할지 모르겠다면 짧은 정의와 함께 용어를 사용하세요.
...
사용자의 의도를 먼저 이해합니다. 현재 대화에 이미 사용자가 캡처하고 싶은 워크플로우가 포함되어 있을 수 있습니다(예: "이걸 스킬로 만들어줘"). 그렇다면 먼저 대화 히스토리에서 사용된 도구, 단계 순서, 사용자가 한 수정, 입출력 형식 등을 추출합니다. 사용자가 빠진 부분을 채워야 할 수도 있으며, 다음 단계로 넘어가기 전에 확인을 받아야 합니다.

1. 이 스킬이 Claude가 무엇을 할 수 있게 해야 하는가?
2. 이 스킬은 언제 트리거되어야 하는가? (어떤 사용자 표현/맥락에서)
3. 예상 출력 형식은 무엇인가?
4. 스킬이 잘 동작하는지 확인하기 위한 테스트 케이스를 설정해야 하는가? 객관적으로 검증 가능한 출력(파일 변환, 데이터 추출, 코드 생성, 고정 워크플로우)을 가진 스킬은 테스트 케이스가 도움이 됩니다. 주관적 출력(글쓰기 스타일, 예술)을 가진 스킬은 보통 테스트 케이스가 필요 없습니다. 스킬 유형에 맞는 기본값을 제안하되 사용자가 결정하도록 합니다.

**인터뷰 및 리서치:**

엣지 케이스, 입출력 형식, 예시 파일, 성공 기준, 의존성에 대해 사전에 적극적으로 질문합니다. 이 부분이 정리될 때까지 테스트 프롬프트 작성을 미룹니다.

사용 가능한 MCP를 확인합니다. 리서치에 유용하다면(문서 검색, 유사 스킬 찾기, 모범 사례 조회), 서브에이전트를 통해 병렬로 리서치하거나(가능한 경우), 아니면 인라인으로 진행합니다. 사용자의 부담을 줄이기 위해 충분한 맥락을 갖추고 준비합니다.
...
- **name**: 스킬 식별자
- **description**: 트리거 시점과 기능. 이것이 주요 트리거 메커니즘입니다. 스킬이 무엇을 하는지와 언제 사용하는지의 구체적인 맥락을 모두 포함합니다. "언제 사용할지"에 관한 모든 정보는 본문이 아닌 여기에 넣습니다. 참고: 현재 Claude는 스킬을 "과소 트리거"하는 경향이 있습니다. 즉, 유용할 때도 사용하지 않는 경향이 있습니다. 이를 방지하기 위해 스킬 설명을 약간 "적극적(pushy)"으로 작성하세요. 예를 들어 "내부 Anthropic 데이터를 표시하기 위한 간단하고 빠른 대시보드 만드는 방법" 대신 "내부 Anthropic 데이터를 표시하기 위한 간단하고 빠른 대시보드 만드는 방법. 사용자가 대시보드, 데이터 시각화, 내부 지표, 또는 어떤 형태의 회사 데이터 표시를 언급할 때마다 이 스킬을 사용하세요. 명시적으로 '대시보드'를 요청하지 않더라도 마찬가지입니다."라고 작성합니다.
...
2. **SKILL.md 본문** — 스킬이 트리거될 때마다 컨텍스트에 있음 (이상적으로는 500줄 미만)
3. **번들 리소스** — 필요에 따라 (무제한, 스크립트는 로드 없이 실행 가능)

이 단어 수는 근사치이며, 필요하다면 더 길게 작성해도 됩니다.

**주요 패턴:**
- SKILL.md를 500줄 미만으로 유지합니다. 이 한계에 가까워지면 추가적인 계층 구조를 추가하고, 스킬을 사용하는 모델이 다음에 어디로 가야 하는지 명확한 포인터를 제공합니다.
- SKILL.md에서 파일을 명확하게 참조하고, 언제 읽어야 하는지 안내합니다.
- 대형 참조 파일(300줄 초과)의 경우 목차를 포함합니다.

**도메인 구성**: 스킬이 여러 도메인/프레임워크를 지원하는 경우, 변형별로 구성합니다:
```
cloud-deploy/
├── SKILL.md (워크플로우 + 선택)
└── references/
...
**놀람 없음의 원칙 (Principle of Lack of Surprise):**

당연한 말이지만, 스킬에는 맬웨어, 익스플로잇 코드, 시스템 보안을 침해할 수 있는 어떤 내용도 포함되어서는 안 됩니다. 스킬의 내용은 설명되었을 때 사용자를 의도적으로 놀라게 해서는 안 됩니다. 오해를 유발하는 스킬이나 무단 접근, 데이터 유출 또는 기타 악의적 활동을 조장하는 스킬 생성 요청에 응해서는 안 됩니다. 단, "XYZ로 롤플레이" 같은 것들은 괜찮습니다.

**작성 패턴:**

지침에서는 명령형을 사용하는 것을 권장합니다.
...
**예시 패턴** — 예시를 포함하는 것이 유용합니다. 다음과 같이 형식을 지정할 수 있습니다("입력"과 "출력"이 예시에 포함된 경우 약간 변형할 수 있습니다):
```markdown
## 커밋 메시지 형식
**예시 1:**
...
모델에게 요청하는 것들의 이유를 가능하면 설명하려고 노력합니다. ALWAYS나 NEVER를 대문자로 쓰거나 매우 경직된 구조를 사용하는 것은 노란 신호입니다. 가능하면 논리를 재구성하고 이유를 설명하여 모델이 왜 그 행동이 중요한지 이해하도록 합니다. 이것이 더 인간적이고 강력하며 효과적인 접근입니다.

**테스트 케이스:**

스킬 초안을 작성한 후, 실제 사용자가 실제로 입력할 만한 현실적인 테스트 프롬프트 2~3개를 만듭니다. 사용자와 공유합니다: [정확히 이 표현을 쓸 필요는 없음] "시도해보고 싶은 테스트 케이스 몇 개입니다. 맞아 보이나요, 아니면 추가하고 싶은 게 있나요?" 그런 다음 실행합니다.

테스트 케이스는 `evals/evals.json`에 저장합니다. 아직 assertion을 작성하지 마세요 — 프롬프트만 저장합니다. assertion은 다음 단계에서 실행이 진행되는 동안 작성할 것입니다.
...
전체 스키마(나중에 추가할 `assertions` 필드 포함)는 `references/schemas.md`를 참조하세요.
...
결과는 스킬 디렉토리와 나란히 `<skill-name>-workspace/`에 저장합니다. 워크스페이스 내에서는 반복(iteration)별로 결과를 구성하고(`iteration-1/`, `iteration-2/` 등), 그 안에서 각 테스트 케이스는 별도 디렉토리를 갖습니다(`eval-0/`, `eval-1/` 등). 모두 미리 만들지 말고, 진행하면서 디렉토리를 생성합니다.

**1단계: 같은 턴에 모든 실행 생성 (스킬 포함 AND 베이스라인)**

각 테스트 케이스마다 같은 턴에 두 개의 서브에이전트를 생성합니다 — 하나는 스킬 포함, 하나는 포함 없이. 중요한 점: 스킬 포함 실행을 먼저 생성하고 나중에 베이스라인을 추가하지 마세요. 모든 것이 비슷한 시간에 완료되도록 한 번에 실행하세요.
...
- 출력 저장 경로: <workspace>/iteration-<N>/eval-<ID>/with_skill/outputs/
- 저장할 출력물: <사용자가 원하는 것 — 예: ".docx 파일", "최종 CSV">
```

**베이스라인 실행** (같은 프롬프트, 단 베이스라인은 맥락에 따라 다름):
- **새 스킬 생성 시**: 스킬 없음. 같은 프롬프트, 스킬 경로 없음, `without_skill/outputs/`에 저장.
- **기존 스킬 개선 시**: 이전 버전 사용. 편집 전에 스킬 스냅샷(`cp -r <skill-path> <workspace>/skill-snapshot/`)을 찍고, 베이스라인 서브에이전트가 스냅샷을 참조하도록 합니다. `old_skill/outputs/`에 저장.

각 테스트 케이스에 대해 `eval_metadata.json`을 작성합니다(assertion은 지금은 비워도 됩니다). 각 eval에는 무엇을 테스트하는지 설명하는 이름을 부여합니다 — "eval-0"과 같은 이름은 피합니다. 이 이름을 디렉토리 이름으로도 사용합니다. 이번 반복에 새로운 또는 수정된 eval 프롬프트가 사용된다면, 새 eval 디렉토리마다 이 파일을 생성합니다.
...
**2단계: 실행 중에 assertion 초안 작성**

실행이 완료될 때까지 그냥 기다리지 마세요 — 이 시간을 생산적으로 활용할 수 있습니다. 각 테스트 케이스에 대한 정량적 assertion을 초안 작성하고 사용자에게 설명합니다. `evals/evals.json`에 이미 assertion이 있다면 검토하고 무엇을 확인하는지 설명합니다.

좋은 assertion은 객관적으로 검증 가능하고 설명적인 이름을 가집니다. 벤치마크 뷰어에서 한눈에 읽어도 각 항목이 무엇을 확인하는지 바로 이해할 수 있어야 합니다. 주관적인 스킬(글쓰기 스타일, 디자인 품질)은 정성적 평가가 더 적합합니다 — 사람의 판단이 필요한 것에 assertion을 억지로 붙이지 마세요.

초안이 완료되면 `eval_metadata.json` 파일과 `evals/evals.json`을 assertion으로 업데이트합니다. 정성적 출력과 정량적 벤치마크 모두 뷰어에서 어떻게 보일지 사용자에게 설명합니다.

**3단계: 실행이 완료되면 타이밍 데이터 저장**

각 서브에이전트 작업이 완료되면 `total_tokens`와 `duration_ms`를 포함하는 알림을 받습니다. 이 데이터를 즉시 실행 디렉토리의 `timing.json`에 저장합니다:
...
이 데이터를 저장할 수 있는 유일한 기회입니다 — 작업 알림을 통해 전달되며 다른 곳에는 저장되지 않습니다. 일괄 처리하려 하지 말고 각 알림이 도착하는 즉시 처리합니다.

**4단계: 채점, 집계, 뷰어 실행**

모든 실행이 완료되면:

1. **각 실행 채점** — 채점 서브에이전트를 생성하거나 인라인으로 채점합니다. `agents/grader.md`를 읽고 각 assertion을 출력물에 대해 평가합니다. 결과를 각 실행 디렉토리의 `grading.json`에 저장합니다. `grading.json`의 expectations 배열은 반드시 `text`, `passed`, `evidence` 필드를 사용해야 합니다(`name`/`met`/`details` 또는 다른 변형이 아님) — 뷰어는 이 정확한 필드 이름에 의존합니다. 프로그래밍 방식으로 확인할 수 있는 assertion은 눈으로 확인하지 말고 스크립트를 작성해 실행하세요 — 스크립트가 더 빠르고 신뢰할 수 있으며 반복 사용 가능합니다.

2. **벤치마크로 집계** — skill-creator 디렉토리에서 집계 스크립트를 실행합니다:
   ```bash
   python -m scripts.aggregate_benchmark <workspace>/iteration-N --skill-name <name>
   ```
   이를 통해 각 구성의 통과율, 시간, 토큰 정보와 평균 ± 표준편차 및 델타가 포함된 `benchmark.json`과 `benchmark.md`가 생성됩니다. `benchmark.json`을 수동으로 생성하는 경우 뷰어가 기대하는 정확한 스키마는 `references/schemas.md`를 참조하세요. 각 `with_skill` 버전을 베이스라인 앞에 배치합니다.

3. **분석가 패스 수행** — 벤치마크 데이터를 읽고 집계 통계가 숨길 수 있는 패턴을 발견합니다. `agents/analyzer.md`(「벤치마크 결과 분석」 섹션)에서 찾아봐야 할 것들 — 스킬 유무에 관계없이 항상 통과하는 assertion(비변별적), 분산이 높은 eval(불안정할 수 있음), 시간/토큰 트레이드오프 등을 확인합니다.

4. **정성적 출력과 정량적 데이터로 뷰어를 실행합니다:**
   ```bash
   ...
   VIEWER_PID=$!
   ```
   반복 2 이상부터는 `--previous-workspace <workspace>/iteration-<N-1>`도 전달합니다.

   **Cowork / 헤드리스 환경:** `webbrowser.open()`을 사용할 수 없거나 디스플레이가 없는 환경이라면 `--static <output_path>`를 사용하여 서버를 시작하는 대신 독립형 HTML 파일을 작성합니다. 사용자가 "모든 리뷰 제출"을 클릭하면 피드백이 `feedback.json` 파일로 다운로드됩니다. 다운로드 후, 다음 반복이 사용할 수 있도록 `feedback.json`을 워크스페이스 디렉토리에 복사합니다.

   참고: 사용자 정의 HTML을 작성하지 말고 `generate_review.py`를 사용하여 뷰어를 생성하세요.

5. **사용자에게 다음과 같이 알립니다:** "브라우저에서 결과를 열었습니다. 탭이 두 개입니다 — '출력물(Outputs)'에서 각 테스트 케이스를 클릭하고 피드백을 남길 수 있고, '벤치마크(Benchmark)'에는 정량적 비교가 표시됩니다. 완료되면 여기로 돌아와 알려주세요."

**뷰어에서 사용자가 보는 것:**

"출력물(Outputs)" 탭은 한 번에 하나의 테스트 케이스를 표시합니다:
- **프롬프트**: 주어진 작업
- **출력물**: 스킬이 생성한 파일, 가능한 경우 인라인으로 렌더링
- **이전 출력물** (반복 2 이상): 마지막 반복의 출력물을 접힌 섹션으로 표시
- **공식 채점** (채점이 실행된 경우): assertion 통과/실패를 접힌 섹션으로 표시
- **피드백**: 입력하는 대로 자동 저장되는 텍스트박스
- **이전 피드백** (반복 2 이상): 마지막 번의 의견을 텍스트박스 아래에 표시

"벤치마크(Benchmark)" 탭은 통계 요약을 표시합니다: 각 구성의 통과율, 타이밍, 토큰 사용량, eval별 분석과 분석가 관찰 사항.

이전/다음 버튼이나 화살표 키로 탐색합니다. 완료되면 "모든 리뷰 제출"을 클릭하면 모든 피드백이 `feedback.json`에 저장됩니다.
...
```json
{
  "reviews": [
    ...
    {"run_id": "eval-2-with_skill", "feedback": "완벽해요, 정말 마음에 들어요", "timestamp": "..."}
  ],
  "status": "complete"
}
```

빈 피드백은 사용자가 괜찮다고 생각한 것입니다. 사용자가 구체적인 불만을 제기한 테스트 케이스에 집중하여 개선합니다.
...
1. **피드백을 일반화하세요.** 여기서 일어나고 있는 큰 그림은, 우리가 수백만 번(어쩌면 문자 그대로, 아니면 그 이상) 다양한 프롬프트에서 사용될 수 있는 스킬을 만들려 한다는 것입니다. 여기서 여러분과 사용자는 속도를 높이기 위해 몇 가지 예시를 반복해서 실행하고 있습니다. 사용자는 이 예시들을 속속들이 알고 있고 새 출력물을 빠르게 평가할 수 있습니다. 하지만 공동으로 개발하는 스킬이 그 예시들에만 작동한다면 쓸모가 없습니다. 사소하고 과적합된 변경이나 지나치게 구속적인 MUST를 넣는 것보다, 고질적인 문제가 있다면 다른 은유나 다른 작업 패턴을 시도해보세요. 시도해보는 비용이 상대적으로 낮고 훌륭한 결과를 얻을 수도 있습니다.

2. **프롬프트를 간결하게 유지하세요.** 효과 없는 것은 제거합니다. 최종 출력물뿐만 아니라 트랜스크립트를 꼭 읽어보세요 — 스킬이 비생산적인 작업에 많은 시간을 낭비하게 만든다면, 그 원인이 되는 스킬 부분을 제거하고 어떻게 되는지 확인해봅니다.

3. **이유를 설명하세요.** 모델에게 요청하는 모든 것의 **이유**를 설명하려고 최대한 노력합니다. 오늘날의 LLM은 *똑똑합니다*. 좋은 프레임워크를 갖추면 단순한 지시를 넘어 이론적 사고력을 발휘하여 정말로 일을 성사시킬 수 있습니다. 사용자의 피드백이 간결하거나 좌절스러워도, 작업을 실제로 이해하고 사용자가 왜 그것을 작성했는지, 실제로 무엇을 원하는지 이해한 다음 그 이해를 지침에 담아냅니다. ALWAYS나 NEVER를 대문자로 쓰거나 매우 경직된 구조를 사용하는 것은 노란 신호입니다 — 가능하다면 논리를 재구성하고 이유를 설명하여 모델이 왜 그것이 중요한지 이해하도록 합니다. 이것이 더 인간적이고 강력하며 효과적인 접근입니다.

4. **테스트 케이스 전반에서 반복적인 작업을 찾으세요.** 테스트 실행의 트랜스크립트를 읽고 서브에이전트들이 모두 비슷한 헬퍼 스크립트를 독립적으로 작성하거나 같은 다단계 접근법을 취했는지 살펴봅니다. 3개의 테스트 케이스 모두에서 서브에이전트가 `create_docx.py`나 `build_chart.py`를 작성했다면, 스킬이 그 스크립트를 번들로 포함해야 한다는 강력한 신호입니다. 한 번 작성하여 `scripts/`에 넣고 스킬이 이를 사용하도록 지시합니다. 이를 통해 미래의 모든 실행이 같은 작업을 반복 발명하는 것을 방지합니다.

이 작업은 꽤 중요합니다(우리는 여기서 연간 수십억 달러의 경제적 가치를 창출하려 하고 있습니다!) 그리고 여러분의 사고 시간이 병목이 아니므로, 충분한 시간을 갖고 깊이 생각해보세요. 수정 초안을 작성한 다음 새 눈으로 다시 보고 개선하는 것을 권장합니다. 사용자의 머릿속에 들어가 정말 원하고 필요한 것이 무엇인지 이해하는 데 최선을 다하세요.
...
1. 스킬에 개선 사항 적용
2. 모든 테스트 케이스를 새 `iteration-<N+1>/` 디렉토리에서 다시 실행합니다. 베이스라인 실행도 포함합니다. 새 스킬을 만드는 경우 베이스라인은 항상 `without_skill`(스킬 없음)입니다 — 반복 전체에서 동일하게 유지됩니다. 기존 스킬을 개선하는 경우 베이스라인으로 무엇이 적합한지 판단합니다: 사용자가 처음 가져온 원래 버전, 또는 이전 반복.
3. 이전 반복 워크스페이스를 `--previous-workspace`로 가리켜 리뷰어를 실행합니다
4. 사용자가 검토하고 완료했다고 알릴 때까지 기다립니다
...
두 버전의 스킬 간 더 엄격한 비교를 원하는 경우(예: 사용자가 "새 버전이 실제로 더 나은가요?"라고 묻는 경우), 블라인드 비교 시스템이 있습니다. 자세한 내용은 `agents/comparator.md`와 `agents/analyzer.md`를 읽어보세요. 기본 아이디어는: 어느 것이 어느 것인지 알려주지 않고 독립적인 에이전트에게 두 출력물을 제공하여 품질을 판단하게 합니다. 그런 다음 왜 승자가 이겼는지 분석합니다.

이것은 선택 사항이며, 서브에이전트가 필요하고, 대부분의 사용자에게는 필요하지 않습니다. 사람의 검토 루프로도 보통 충분합니다.

---

**설명 최적화:**

SKILL.md 프론트매터의 description 필드는 Claude가 스킬을 호출할지 결정하는 주요 메커니즘입니다. 스킬을 만들거나 개선한 후, 더 나은 트리거링 정확도를 위한 설명 최적화를 제안합니다.

**1단계: 트리거 평가 쿼리 생성**

트리거해야 하는 것과 트리거하지 않아야 하는 것이 혼합된 20개의 평가 쿼리를 만듭니다. JSON으로 저장합니다:
...
쿼리는 현실적이어야 하며, Claude Code 또는 Claude.ai 사용자가 실제로 입력할 내용이어야 합니다. 추상적인 요청이 아니라 구체적이고 상세한 요청이어야 합니다. 예를 들어, 파일 경로, 직업이나 상황에 대한 개인적인 맥락, 열 이름과 값, 회사 이름, URL, 약간의 배경 이야기가 포함될 수 있습니다. 일부는 소문자이거나 약어, 오타, 구어체가 포함될 수 있습니다. 다양한 길이를 사용하고, 명백한 것보다 엣지 케이스에 집중합니다(사용자가 검토할 기회가 있습니다).

나쁜 예: `"이 데이터를 형식화해줘"`, `"PDF에서 텍스트 추출해줘"`, `"차트 만들어줘"`

좋은 예: `"그러니까 제 상사가 방금 이 xlsx 파일을 보냈는데요 (다운로드 폴더에 있고, 이름이 'Q4 sales final FINAL v2.xlsx' 같은 것 같아요), 수익 마진을 퍼센트로 보여주는 열을 추가하길 원해요. 수익은 C 열에 있고 비용은 D 열에 있는 것 같아요"`

**should-trigger 쿼리**(8~10개)의 경우, 커버리지를 생각합니다. 같은 의도를 다양한 방식으로 표현합니다 — 일부는 공식적으로, 일부는 비공식적으로. 사용자가 스킬 이름이나 파일 유형을 명시적으로 언급하지 않지만 명확히 필요로 하는 경우도 포함합니다. 드문 사용 사례와 이 스킬이 다른 스킬과 경쟁하지만 이겨야 하는 경우도 포함합니다.

**should-not-trigger 쿼리**(8~10개)의 경우, 가장 가치 있는 것은 근접 오류(near-miss)입니다 — 스킬과 키워드나 개념을 공유하지만 실제로는 다른 것이 필요한 쿼리입니다. 인접 도메인, 단순 키워드 매칭이라면 트리거되지만 실제로는 그래서는 안 되는 애매한 표현, 그리고 쿼리가 스킬이 하는 것에 관련되지만 다른 도구가 더 적합한 맥락의 경우를 생각해봅니다.

피해야 할 핵심 사항: should-not-trigger 쿼리를 명백히 무관하게 만들지 마세요. PDF 스킬의 부정적 테스트로 "피보나치 함수 작성해줘"는 너무 쉽습니다 — 아무것도 테스트하지 않습니다. 부정적 케이스는 진정으로 까다로워야 합니다.

**2단계: 사용자와 검토**

HTML 템플릿을 사용하여 사용자가 검토할 수 있도록 eval 세트를 제공합니다:

1. `assets/eval_review.html`에서 템플릿을 읽습니다
2. 플레이스홀더를 교체합니다:
   - `__EVAL_DATA_PLACEHOLDER__` → eval 항목의 JSON 배열 (따옴표 없이 — JS 변수 할당임)
   - `__SKILL_NAME_PLACEHOLDER__` → 스킬 이름
   - `__SKILL_DESCRIPTION_PLACEHOLDER__` → 스킬의 현재 설명
3. 임시 파일에 저장하고 열기: `open /tmp/eval_review_<skill-name>.html`
4. 사용자는 쿼리를 수정하고, should-trigger를 전환하고, 항목을 추가/제거한 다음 "Eval 세트 내보내기"를 클릭할 수 있습니다
5. 파일은 `~/Downloads/eval_set.json`으로 다운로드됩니다 — 여러 버전이 있을 경우(예: `eval_set (1).json`) 가장 최근 버전을 Downloads 폴더에서 확인합니다

이 단계는 중요합니다 — 잘못된 eval 쿼리는 잘못된 설명으로 이어집니다.

**3단계: 최적화 루프 실행**

사용자에게 말합니다: "이 작업은 시간이 걸립니다 — 백그라운드에서 최적화 루프를 실행하고 주기적으로 확인하겠습니다."

eval 세트를 워크스페이스에 저장한 다음 백그라운드에서 실행합니다:
...
현재 세션을 구동하는 시스템 프롬프트의 모델 ID를 사용합니다. 이를 통해 트리거링 테스트가 사용자가 실제로 경험하는 것과 일치합니다.

실행 중에는 주기적으로 출력을 확인하여 현재 몇 번째 반복인지와 점수가 어떤지 사용자에게 업데이트합니다.

이 스크립트는 전체 최적화 루프를 자동으로 처리합니다. eval 세트를 60% 훈련 세트와 40% 홀드아웃 테스트 세트로 분할하고, 현재 설명을 평가(신뢰할 수 있는 트리거 비율을 얻기 위해 각 쿼리를 3번 실행)한 다음, 실패한 것을 기반으로 Claude에게 확장 사고로 개선을 제안하도록 요청합니다. 훈련 세트와 테스트 세트 모두에서 각 새 설명을 재평가하고 최대 5번 반복합니다. 완료되면 결과를 브라우저에서 HTML 보고서로 열어 반복당 결과를 보여주고, `best_description`이 포함된 JSON을 반환합니다 — 과적합을 방지하기 위해 훈련 점수가 아닌 테스트 점수로 선택됩니다.

**스킬 트리거링 작동 방식:**

트리거링 메커니즘을 이해하면 더 나은 eval 쿼리를 설계하는 데 도움이 됩니다. 스킬은 Claude의 `available_skills` 목록에 이름 + 설명과 함께 나타나며, Claude는 그 설명을 바탕으로 스킬을 참조할지 결정합니다. 중요한 점은 Claude가 스스로 쉽게 처리할 수 없는 작업에만 스킬을 참조한다는 것입니다 — "이 PDF 읽어줘"와 같은 단순하고 한 단계짜리 쿼리는 설명이 완벽하게 일치하더라도 스킬을 트리거하지 않을 수 있습니다. Claude가 기본 도구로 직접 처리할 수 있기 때문입니다. 복잡하고 다단계이거나 전문적인 쿼리는 설명이 일치할 때 안정적으로 스킬을 트리거합니다.

이는 eval 쿼리가 Claude가 스킬을 참조함으로써 실제로 이점을 얻을 만큼 충분히 실질적이어야 함을 의미합니다. "파일 X 읽어줘"와 같은 단순한 쿼리는 설명 품질에 관계없이 스킬을 트리거하지 않으므로 좋은 테스트 케이스가 아닙니다.

**4단계: 결과 적용**

JSON 출력에서 `best_description`을 가져와 스킬의 SKILL.md 프론트매터를 업데이트합니다. 이전/이후를 사용자에게 보여주고 점수를 보고합니다.

---

**패키징 및 제출 (present_files 도구가 있는 경우에만):**

`present_files` 도구에 접근 가능한지 확인합니다. 없다면 이 단계를 건너뜁니다. 있다면 스킬을 패키징하고 사용자에게 .skill 파일을 제공합니다:
...
Claude.ai에서는 핵심 워크플로우가 동일합니다(초안 → 테스트 → 검토 → 개선 → 반복). 단, Claude.ai에는 서브에이전트가 없으므로 일부 메커니즘이 달라집니다. 조정해야 할 사항:

**테스트 케이스 실행**: 서브에이전트가 없으므로 병렬 실행이 불가능합니다. 각 테스트 케이스마다 스킬의 SKILL.md를 읽은 다음 그 지침에 따라 직접 테스트 프롬프트를 수행합니다. 하나씩 순서대로 진행합니다. 이 방법은 독립적인 서브에이전트보다 엄밀하지 않지만(스킬을 작성한 사람이 동시에 실행하므로 전체 맥락을 갖고 있음), 유용한 건전성 검사 역할을 합니다 — 그리고 사람의 검토 단계가 이를 보완합니다. 베이스라인 실행은 건너뜁니다 — 스킬을 사용하여 요청된 작업을 완료합니다.

**결과 검토**: 브라우저를 열 수 없는 경우(예: Claude.ai의 VM에 디스플레이가 없거나 원격 서버인 경우), 브라우저 리뷰어를 완전히 건너뜁니다. 대신 대화에서 직접 결과를 제시합니다. 각 테스트 케이스에 대해 프롬프트와 출력물을 보여줍니다. 출력물이 사용자가 봐야 하는 파일(.docx나 .xlsx 등)인 경우 파일 시스템에 저장하고 다운로드해서 검토할 수 있도록 위치를 알려줍니다. 인라인으로 피드백을 요청합니다: "어떻게 보이나요? 변경하고 싶은 것이 있나요?"

**벤치마킹**: 정량적 벤치마킹을 건너뜁니다 — 서브에이전트 없이는 베이스라인 비교가 의미 없습니다. 사용자의 정성적 피드백에 집중합니다.

**반복 루프**: 이전과 동일합니다 — 스킬 개선, 테스트 케이스 재실행, 피드백 요청 — 단, 중간에 브라우저 리뷰어 없이 진행합니다. 파일 시스템이 있는 경우 여전히 반복 디렉토리에 결과를 구성할 수 있습니다.

**설명 최적화**: 이 섹션은 Claude Code에서만 사용 가능한 `claude` CLI 도구(특히 `claude -p`)가 필요합니다. Claude.ai에서는 건너뜁니다.

**블라인드 비교**: 서브에이전트가 필요합니다. 건너뜁니다.

**패키징**: `package_skill.py` 스크립트는 Python과 파일 시스템이 있는 어디서나 작동합니다. Claude.ai에서 실행하고 사용자가 결과 `.skill` 파일을 다운로드할 수 있습니다.
...
- 서브에이전트가 있으므로 주요 워크플로우(테스트 케이스 병렬 생성, 베이스라인 실행, 채점 등)가 모두 작동합니다. (단, 타임아웃으로 심각한 문제가 발생하면 병렬 대신 순차적으로 테스트 프롬프트를 실행해도 됩니다.)
- 브라우저나 디스플레이가 없으므로, eval 뷰어를 생성할 때 서버를 시작하는 대신 `--static <output_path>`를 사용하여 독립형 HTML 파일을 작성합니다. 그런 다음 사용자가 클릭하여 브라우저에서 HTML을 열 수 있는 링크를 제공합니다.
- 어떤 이유로 Cowork 설정이 테스트 실행 후 Claude가 eval 뷰어를 생성하지 않으려는 경향이 있으므로, 다시 강조합니다: Claude Code에서든 Cowork에서든, 테스트 실행 후에는 항상 `generate_review.py`를 사용하여 eval 뷰어를 생성하여 사람이 예시를 검토하고 난 다음에 스킬을 수정해야 합니다. 사전에 미리 강조할게요: **직접 입력을 평가하기 *전에* eval 뷰어를 생성하세요.** 가능한 빨리 사람 앞에 보여줘야 합니다!
- 피드백은 다르게 작동합니다: 실행 중인 서버가 없으므로, 뷰어의 "모든 리뷰 제출" 버튼이 `feedback.json` 파일을 다운로드합니다. 그런 다음 거기서 읽을 수 있습니다(먼저 접근 권한을 요청해야 할 수도 있습니다).
- 패키징은 작동합니다 — `package_skill.py`는 Python과 파일 시스템만 필요합니다.
- 설명 최적화(`run_loop.py` / `run_eval.py`)는 브라우저가 아닌 subprocess를 통해 `claude -p`를 사용하므로 Cowork에서 잘 작동해야 합니다. 단, 스킬이 완전히 완성되고 사용자가 좋다고 동의한 후까지 저장해두세요.

---

**참조 파일:**

`agents/` 디렉토리에는 특수 서브에이전트를 위한 지침이 있습니다. 관련 서브에이전트를 생성해야 할 때 읽어보세요.

- `agents/grader.md` — 출력물에 대한 assertion 평가 방법
- `agents/comparator.md` — 두 출력물 간 블라인드 A/B 비교 방법
...
- 스킬이 무엇에 관한 것인지 파악하기
- 스킬 초안 작성 또는 편집하기
- 테스트 프롬프트로 '스킬이 탑재된 Claude' 실행하기
- 사용자와 함께 출력물 평가하기:
  - `benchmark.json`을 생성하고 `eval-viewer/generate_review.py`를 실행하여 사용자가 검토하도록 도움
  - 정량적 evals 실행
- 여러분과 사용자가 만족할 때까지 반복하기
- 최종 스킬을 패키징하여 사용자에게 반환하기

할 일 목록이 있다면 잊지 않도록 단계들을 추가하세요. Cowork에 있다면 특히 "eval JSON을 생성하고 사람이 테스트 케이스를 검토할 수 있도록 `eval-viewer/generate_review.py` 실행하기"를 할 일 목록에 추가하여 반드시 실행되도록 하세요.
